...
//    }
    ...
    if (self.view.superview != nil)
    {
        ...
        r = CGRectMake((size.width-kWindowWidth)/2, (size.height-kWindowHeight)/2, kWindowWidth, kWindowHeight);
    }
    else
    {
        // View is not visible, position outside screen bounds
        r = CGRectMake((size.width-kWindowWidth)/2, -kWindowHeight, kWindowWidth, kWindowHeight);
    }
...
- (LCButton *)addButton:(NSString *)title
{
    // Update view height
    kWindowHeight += 45.0;
...
- (LCButton *)addButton:(NSString *)title actionBlock:(ActionBlock)action
{
    LCButton *btn = [self addButton:title];
...
- (LCButton *)addButton:(NSString *)title target:(id)target selector:(SEL)selector
{
    LCButton *btn = [self addButton:title];
...
- (void)buttonTapped:(LCButton *)btn
{
...
-(LCAlertViewResponder *)showTitle:(UIViewController *)vc title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    self.view.alpha = 0;
    ...
    switch (style)
    {
        case LCAlertSuccess:
            ...
            iconImage = LCAlertViewStyle.imageOfCheckmark;
            break;

        case LCAlertError:
            viewColor = UIColorFromRGB(0xC1272D);
            iconImage = LCAlertViewStyle.imageOfCross;
            break;

        case LCAlertNotice:
            viewColor = UIColorFromRGB(0x727375);
            iconImage = LCAlertViewStyle.imageOfNotice;
            break;

        case LCAlertWarning:
            viewColor = UIColorFromRGB(0xFFD110);
            iconImage = LCAlertViewStyle.imageOfWarning;
            break;

        case LCAlertInfo:
            viewColor = UIColorFromRGB(0x2866BF);
            iconImage = LCAlertViewStyle.imageOfInfo;
            break;

        case LCAlertEdit:
            viewColor = UIColorFromRGB(0xA429FF);
            iconImage = LCAlertViewStyle.imageOfEdit;
            break;
    }
    if (self.themeColor) {
       viewColor = self.themeColor;
    }
    ...
    // Chainable objects
    return [[LCAlertViewResponder alloc] init:self];
}
...
{
    [self showTitle:vc title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:LCAlertSuccess];
}
...
{
    [self showTitle:vc title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:LCAlertError];
}
...
{
    [self showTitle:vc title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:LCAlertNotice];
}
...
{
    [self showTitle:vc title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:LCAlertWarning];
}
...
{
    [self showTitle:vc title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:LCAlertInfo];
}
...
{
    [self showTitle:vc title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:LCAlertEdit];
}
